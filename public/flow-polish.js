(() => {
 ...
 window.flowButtonEditor=(host,n,rerender)=>{
  ...
  if(n.choices.length){group.append(el('p','⑂ Até 13 respostas rápidas, cada uma com seu caminho.','small muted'));n.choices.forEach((c,i)=>{const row=el('div',null,'fp-button-row');group.append(row);field(row,'Texto da resposta '+(i+1),c,'title',refresh);row.append(button('Remover resposta',()=>{if(c.next){toast('Exclua a conexão desta resposta antes de remover o botão.');return;}remember();n.choices.splice(i,1);refresh();},'subtle'));});if(n.choices.length<13)group.append(button('+ Resposta',()=>{remember();n.choices.push({title:'',next:''});refresh();},'outline'));group.append(el('small','Para enviar links, adicione uma mensagem de link no próximo passo.','muted'));return;}
