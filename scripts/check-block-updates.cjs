...
(async()=>{
 ...
 try{
  ...
  const duplicated=await page.evaluate(()=>{const original=mapState.map.nodes[0];const copy=mapState.map.nodes.at(-1);const retained=copy.links[0].title===original.links[0].title&&copy.next===original.next;copy.links[0].title='Outro';return {retained,independent:original.links[0].title==='Receber 😊',ids:copy.id!==original.id,numbers:copy.number!==original.number};});assert.deepEqual(duplicated,{retained:true,independent:true,ids:true,numbers:true});
