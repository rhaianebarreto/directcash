...
export type Rule={id:string;name:string;trigger:string;media_id:string;keywords:string;message:string;link:string;public_reply:string;active:number;created:number};
export function validateRule(b:Record<string,unknown>):Omit<Rule,'id'|'created'> {
  ...
  const name=field('name',80), trigger=field('trigger',20), media_id=field('media_id',100), keywords=field('keywords',160), message=field('message',600), link=field('link',350), public_reply=field('public_reply',300);
  if(!name||!message||!keywords||!['comment','dm'].includes(trigger)||!safeLink(link)||typeof b.active!=='boolean')throw Error('Preencha nome, palavra-chave, mensagem e um link HTTPS válido.');
  if(trigger==='comment'&&!/^\d+$/.test(media_id))throw Error('Escolha um post para a automação de comentários.');
  return {name,trigger,media_id:trigger==='comment'?media_id:'',keywords,message,link,public_reply:trigger==='comment'?public_reply:'',active:b.active?1:0};
}
