...
(() => {
 ...
 async function prepare(file,kind,convert){
  ...
  if(kind==='audio'){
   ...
   }catch(e){if(convert)throw Error(e.message||'Este navegador não consegue converter este áudio. Use MP3 ou M4A.');}finally{await ctx?.close();}
  }
 ...
 async function upload(file,kind,convert){const prepared=await prepare(file,kind,convert);const r=await fetch('/api/uploads?kind='+kind,{method:'POST',headers:{'Content-Type':prepared.type},body:prepared.blob}),data=await r.json();if(!r.ok)throw Error(data.error||'Falha ao anexar o arquivo.');return {...data,duration:prepared.duration,size:prepared.blob.size,name:convert?file.name.replace(/\.[^.]+$/,'.wav'):file.name};}
 function attachmentFields(host,kind,url,onupload){
  ...
  if(kind==='audio'){const label=el('label','Converter para WAV compatível'),help=el('small','Para OGG, a conversão é ativada ao escolher o arquivo. Depende do suporte do navegador. O áudio será enviado como áudio.','muted');label.prepend(convert);box.append(label,help);}
  ...
  input.onchange=async()=>{const file=input.files?.[0];if(!file)return;if(kind==='audio'&&/\.ogg$/i.test(file.name))convert.checked=true;attach.disabled=true;status.textContent='Preparando '+file.name+'…';try{const result=await upload(file,kind,convert.checked);onupload(result);status.textContent='Arquivo pronto.';}catch(e){status.textContent=e.message||'Não foi possível ler este áudio. Use MP3 ou M4A.';}finally{attach.disabled=false;input.value='';}};
 }
