...
import {licenseFor,activateLicense} from './license';
...
const body=async(req:Request)=>JSON.parse(await boundedText(req,16384)) as Record<string,unknown>;
...
export default {
  async fetch(req:Request,env:AppEnv,ctx:ExecutionContext):Promise<Response>{try{return secure(await handle(req,env,ctx));}catch(e){console.error(JSON.stringify({event:'request_failed',type:e instanceof Error?e.name:'unknown'}));return secure(json({error:'Não foi possível concluir. Confira a configuração e tente novamente.'},500));}},
  async scheduled(_event:ScheduledController,env:AppEnv,ctx:ExecutionContext){ctx.waitUntil(maintenance(env).catch(()=>console.error(JSON.stringify({event:'maintenance_failed'}))));}
...
async function handle(req:Request,env:AppEnv,ctx:ExecutionContext):Promise<Response>{
  ...
  if(path==='/webhook'){
    ...
    try{await ingest(env,JSON.parse(text));}catch(e){if(e instanceof SyntaxError)return json({},400);throw e;}
    ctx.waitUntil(drain(env).catch(()=>console.error(JSON.stringify({event:'drain_failed'}))));return json({received:true});
  }
  ...
  if(path==='/api/status'&&req.method==='GET'){
    ...
    return json({license:license?{student:license.student,expires:license.expires}:null,account:a?{id:a.id,username:a.username,expires:a.expires}:null,configured:!!cfg,appId:cfg?.appId||'',owner:cfg?.owner||'',contact:cfg?.contact||'',verifyToken:cfg?.verifyToken||'',callback:url.origin+'/oauth/callback',webhook:url.origin+'/webhook',privacy:url.origin+'/privacy.html',lastWebhook:last?Number(last.value):null,counts});
  }
  if(path==='/api/license'&&req.method==='POST'){
    ...
    try{if(!await activateLicense(env,b.code.trim().toUpperCase(),a.id))return json({error:'Licença inválida, suspensa, vencida ou emitida para outro Instagram.'},400);}catch{return json({error:'Servidor de licenças indisponível. Tente novamente em instantes.'},503);}
    return json({ok:true});
  }
  if(path==='/api/settings'&&req.method==='POST'){
    if(await account(env))return json({error:'Desconecte o Instagram antes de trocar o aplicativo Meta.'},409);
  ...
  if(path==='/oauth/start'&&req.method==='GET'){
    ...
    const state=crypto.randomUUID();await env.DB.prepare('INSERT INTO oauth(id,session,expires) VALUES(?,?,?)').bind(await digest(state),s.id,now()+600).run();
  ...
  if(path==='/oauth/callback'&&req.method==='GET'){
    ...
    const row=await env.DB.prepare('DELETE FROM oauth WHERE id=? AND session=? AND expires>? RETURNING id').bind(await digest(state),s.id,now()).first();
    ...
    try{
      ...
      const existing=await account(env);if(existing&&existing.id!==a.id)return json({error:'Desconecte a conta atual antes de conectar outra.'},409);
      ...
      return Response.redirect(url.origin+'/?connection=ok#setup',302);
  ...
  if(path==='/api/disconnect'&&req.method==='POST'){
    await env.DB.batch([env.DB.prepare('DELETE FROM account'),env.DB.prepare('UPDATE rules SET active=0'),env.DB.prepare('DELETE FROM jobs'),env.DB.prepare('DELETE FROM events'),env.DB.prepare('DELETE FROM oauth'),env.DB.prepare("DELETE FROM settings WHERE key='last_webhook'")]);return json({ok:true});
  }
  ...
  if(path==='/api/media'&&req.method==='GET'){
    ...
    try{const data=await graph(env,a,`${a.id}/media?fields=id,caption,permalink,media_type&limit=25${after?'&after='+encodeURIComponent(after):''}`);return json({data:data.data||[],after:data.paging?.next?data.paging?.cursors?.after:null});}catch{return json({error:'Não foi possível listar os posts. Confira a conexão.'},400);}
  }
  ...
  if(path==='/api/rules'&&req.method==='POST'){
    ...
    if(r.active&&(!a||!await licenseFor(env,a.id)))return json({error:'Conecte o Instagram e ative sua licença antes de ativar automações.'},400);
    ...
    await env.DB.prepare('INSERT INTO rules(id,name,trigger,media_id,keywords,message,link,public_reply,active,created) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,trigger=excluded.trigger,media_id=excluded.media_id,keywords=excluded.keywords,message=excluded.message,link=excluded.link,public_reply=excluded.public_reply,active=excluded.active').bind(id,r.name,r.trigger,r.media_id,r.keywords,r.message,r.link,r.public_reply,r.active,now()).run();return json({id});
  }
  ...
  if(path==='/api/activity'&&req.method==='GET')return json({jobs:(await env.DB.prepare('SELECT id,kind,status,created,updated,detail FROM jobs ORDER BY created DESC LIMIT 50').all()).results,events:(await env.DB.prepare('SELECT kind,detail,created FROM events ORDER BY created DESC LIMIT 20').all()).results});
